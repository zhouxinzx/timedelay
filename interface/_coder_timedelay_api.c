 ...
 * Return Type  : void
 */
void timedelay_api(const mxArray * const prhs[2], const mxArray *plhs[1])
{
  real_T (*x_data)[3000];
  int32_T x_size[2];
  real_T (*y_data)[3000];
  ...
  emlrt_marshallIn(&st, emlrtAlias((const mxArray *)prhs[0]), "x", (real_T **)
                   &x_data, x_size);
  emlrt_marshallIn(&st, emlrtAlias((const mxArray *)prhs[1]), "y", (real_T **)
                   &y_data, y_size);
  ...
  plhs[0] = emlrt_marshallOut(time_delay);
}
...
/*
 * File trailer for _coder_timedelay_api.c
 *
 * [EOF]
 */
